...
export default defineWidget('CellFormatter', false, {
        ...
        for (rowObj of this._grid._dataSource._pageObjs) {
            ...
            for (rule of this.rules) {
                ...
                if (rule.ruleNanoflow && rule.ruleNanoflow.nanoflow) {
                    this._evalNanoFlow(rule.ruleNanoflow, rowObj, callbackFunc(nodeToApply, rule));
                } else {
                    const result = this._evalJS(rule.js, rowObj);
                    this._handleResults(result, nodeToApply, rule);
                }
            }
        }
    },
    ...
    _handleResults(result, nodeToApply, column) {
        if (result) {
            ...
            construct.create("div", { innerHTML: result, className: column.columnName + '-html' }, nodeToApply, column.position);
